...
, susp AS (
    SELECT DISTINCT
        ...
        , IF(
            ...
            AND (accept_longitude = store_longitude)
            , TRUE, FALSE) AS same_accept_store_latlong     -- DSYS001
    FROM susp_1 o                                                              
    ORDER BY order_date
)
...
, all_fgps AS (
    SELECT 
        ...
        , CASE 
            WHEN same_accept_store_latlong 
                AND count_order_same_acc_store_latlong >= 1 
                THEN 1 
            ELSE 0 
            END AS dsys001
    FROM susp a
...
SELECT DISTINCT 
    ...
    , same_accept_store_latlong AS is_001                -- DSYS001
    , count_order_same_acc_store_latlong AS cnt_001      -- DSYS001
