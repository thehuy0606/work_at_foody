...
, susp_1 AS (
    SELECT
        ...
        , ST_DISTANCE(
            ...
            , TO_SPHERICAL_GEOGRAPHY(ST_POINT(next_longitude, next_latitude))
            )/1000 AS distance
        , latitude
        , longitude
        , next_accuracy
    ...
    SELECT 
        ...
        , IF(
            COUNT(
                CASE 
                    WHEN distance > 1 
                        AND next_accuracy < 20 
                        AND DATE_DIFF('second', ping_time, next_upload) = 5 
                        THEN o.order_id 
                    END
            ) >= 1
            , TRUE, FALSE) AS jump_1km_accuracy_20_within5s     -- DSYS006                                                                            
    FROM susp_1 o                                                              
    GROUP BY 1,2,3,4
    ORDER BY order_date
)
...
, all_fgps AS (
    SELECT 
        ...
        , CASE 
            WHEN jump_1km_accuracy_20_within5s 
                AND count_jump_1km_accuracy_20_within5s >= 3 
                THEN 1 
            ELSE 0 
            END AS dsys006     
    FROM susp a
...
SELECT DISTINCT 
    ...
    , jump_1km_accuracy_20_within5s AS is_006            -- DSYS006
    , count_jump_1km_accuracy_20_within5s AS cnt_006     -- DSYS006
