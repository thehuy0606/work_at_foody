...
, final_data AS (
    SELECT DISTINCT
        ...
        -- , MOD(gc.speed_kmh*10000, 1) AS modspeed
        , LENGTH(SPLIT_PART(SPLIT_PART(SPLIT_PART(CAST(speed_kmh AS VARCHAR), '.', 2), '0E', 1), 'E', 1)) + CAST(SPLIT_PART(CAST(speed_kmh AS VARCHAR), 'E', 2) AS BIGINT) * -1 AS speed_digit
        , o.service_type
...
, susp_1 AS (
    SELECT
        ...
        , upload_time 
        , service_type                                                                                         
        , LEAD(count_acc_dec5, 5) 
            OVER(
                PARTITION BY order_id 
                ORDER BY ping_time, upload_time
            ) AS lead_5_ping_acc_dec5
        , count_acc_dec5
    FROM (
        SELECT DISTINCT
            order_date
...
, susp AS (
    SELECT 
        ...
        , IF(
            COUNT(
                CASE 
                    WHEN (lead_5_ping_acc_dec5 - count_acc_dec5 = 5) 
                        THEN o.order_id 
                END) >= 1 
            , TRUE, FALSE) AS repet_acc_dec5
    FROM susp_1 o
...
SELECT 
    ...
    , repet_acc_dec5 AS is_215                              -- DSYS215
    , count_repet_acc_dec5 AS cnt_215                       -- DSYS215
