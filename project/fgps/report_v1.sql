...
fgps AS 
    (
        SELECT  -- *
            ...
            ,   driver_spe_id
            ,   driver_name 
            ,   IF(dsys209 + dsys211 + dsys212 + dsys213 > 0, 1, 0) is_rule_repetitive_digit_acc_or_speed 
            ,   IF(dsys109 + dsys111 + dsys112 + dsys113 > 0, 1, 0) is_rule_repetitive_same_accuracy 
            ,   IF(dsys006 > 0, 1, 0)                               is_rule_jump_lat_long 
            ,   IF(dsys016 + dsys017 > 0, 1, 0)                     is_rule_repetitive_high_accuracy 
            ,   IF(dsys008 > 0, 1, 0)                               is_rule_repetitive_same_speed 
            ,   IF(dsys010 > 0, 1, 0)                               is_rule_suspicious_lat_long 
            ,   IF(dsys001 + dsys004 > 0, 1, 0)                     is_rule_same_accept_lat_long 
            ,   IF(dsys214 + dsys215 > 0, 1, 0)                     is_proxy_repetitive_digit_acc_or_speed 
            ,   IF(dsys114 + dsys115 > 0, 1, 0)                     is_proxy_repetitive_same_accuracy 
            ,   IF(isproxy > 0, 1, 0)                               is_proxy_repacked_app_usage 
        FROM    base 
        WHERE   1=1
...
tab_a AS 
    (
        ...
        WHERE   1=1 
            AND ( 
                    is_rule_repetitive_digit_acc_or_speed   + 
                    is_rule_repetitive_same_accuracy        +
                    is_rule_jump_lat_long                   + 
                    is_rule_repetitive_high_accuracy        + 
                    is_rule_repetitive_same_speed           + 
                    is_rule_suspicious_lat_long             + 
                    is_rule_same_accept_lat_long            > 0 
                ) 
            AND (is_proxy_repetitive_digit_acc_or_speed + is_proxy_repetitive_same_accuracy + is_proxy_repacked_app_usage = 0) 
    )
select * from tab_a 
