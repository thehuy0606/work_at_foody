-- DROP TABLE IF EXISTS    dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__pickup_after;
-- CREATE TABLE            dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__pickup_after WITH (partitioned_by = ARRAY['grass_date']) AS 
DELETE FROM dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__pickup_after WHERE grass_date BETWEEN DATE(DATE_PARSE('${D7}','%Y%m%d')) AND DATE(DATE_PARSE('${yesterday}','%Y%m%d'));
INSERT INTO dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__pickup_after 
...
final_data AS  
    (
        ...
        LEFT JOIN   dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__driver_track_ping_di  gc 
            ...
            AND gc.ping_time    BETWEEN od.delivery_pickup_time AND od.delivery_delivered_time 
        WHERE   1=1 
