-- DROP TABLE IF EXISTS    dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__delive_after;
-- CREATE TABLE            dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__delive_after WITH (partitioned_by = ARRAY['grass_date']) AS 
DELETE FROM dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__delive_after WHERE grass_date BETWEEN DATE(DATE_PARSE('${D7}','%Y%m%d')) AND DATE(DATE_PARSE('${yesterday}','%Y%m%d'));
INSERT INTO dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__delive_after 
...
final_data AS  
    (
        ...
        LEFT JOIN   dev_vnfdbi_opsndrivers.bnp_bi_fraud__fgps__driver_track_ping_di  gc 
            ...
            AND gc.ping_time    BETWEEN od.delivery_delivered_time AND od.delivery_delivered_time + INTERVAL '5' MINUTE 
        WHERE   1=1 
            AND od.order_date   BETWEEN DATE(DATE_PARSE('${D7}','%Y%m%d'))                          AND DATE(DATE_PARSE('${yesterday}','%Y%m%d')) 
            AND gc.ping_date    BETWEEN DATE(DATE_PARSE('${D7}','%Y%m%d'))  - INTERVAL '1' DAY      AND DATE(DATE_PARSE('${yesterday}','%Y%m%d')) + INTERVAL '1' DAY
